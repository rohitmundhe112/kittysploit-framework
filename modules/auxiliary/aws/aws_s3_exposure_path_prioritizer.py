...
class Module(Auxiliary, Http_client):
    __info__ = {
        "name": "AWS S3 Exposure Path Prioritizer",
        "author": ["KittySploit Team"],
        "description": "Correlate AWS S3 findings into prioritized exposure-to-impact paths.",
        "tags": ["aws", "s3", "correlation", "attack-path", "prioritization"],
    }

    target = OptString("", "S3 bucket name or URL (for auto-collection mode)", required=False)
    ...
    exposure_file = OptString("", "JSON from s3_exposure_audit", required=False)
    list_file = OptString("", "JSON from s3_bucket_file_list", required=False)
    sensitive_file = OptString("", "JSON from s3_sensitive_pattern_scan", required=False)
    ...
    def _auto_collect(self, bucket, timeout_seconds, max_list_files, max_quick_scan_files, max_bytes):
        ...
        # Exposure check.
        test_url = f"https://{bucket}.s3.amazonaws.com/?list-type=2&max-keys=1"
        ...
        for n in names[:max_quick_scan_files]:
            u = f"https://{bucket}.s3.amazonaws.com/{quote(n, safe='/')}"
    ...
    def run(self):
        ...
        if self.auto_collect and not any([exposure, listing, sensitive]):
            ...
            if bucket:
                print_info(f"Auto-collecting S3 signals for {bucket} ...")
        ...
        bucket = self._normalize_bucket(listing.get("bucket") or self.target) or "s3-bucket"
        ...
        if public_listing > 0:
            paths.append({
                ...
                "reason": "Anonymous listing exposes S3 object inventory at scale.",
            })
        if flagged > 0:
            paths.append({
                ...
                "reason": "Sensitive patterns were detected in downloadable S3 objects.",
            })
        ...
        print_success(f"AWS S3 exposure paths: {len(paths)} (risk={risk_level}/{risk_score})")
    ...
    def get_graph_nodes(self, data):
        if not isinstance(data, dict) or "error" in data:
            return [], []
        root = data.get("bucket", "s3-bucket")
        nodes = [{
            "id": root,
