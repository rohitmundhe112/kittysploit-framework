...
class Module(Auxiliary, Http_client):
    __info__ = {
        "name": "AWS S3 Bucket File List",
        "author": ["KittySploit Team"],
        "description": "List files from an AWS S3 bucket using anonymous access.",
        "tags": ["aws", "s3", "cloud", "enumeration"],
    }

    target = OptString("", "S3 bucket name or URL (e.g. my-bucket or https://my-bucket.s3.amazonaws.com)", required=True)
    ...
    def run(self):
        ...
        print_info(f"Listing S3 objects from {bucket}")
        continuation = ""
        collected = []
        errors = []
        page_count = 0
    ...
    def get_graph_nodes(self, data):
        ...
        for i, f in enumerate(data.get("files", [])[:40]):
            nid = f"s3_{i}_{data.get('bucket', 'b')}"
            nodes.append({
                "id": nid,
            ...
            edges.append({
                ...
                "to": nid,
                "label": "object",
                "custom_info": "S3 object in bucket",
            })
        return nodes, edges
