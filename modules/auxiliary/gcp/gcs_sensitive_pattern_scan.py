...
class Module(Auxiliary, Http_client):
    __info__ = {
        "name": "GCP GCS Sensitive Pattern Scan",
        "author": ["KittySploit Team"],
        "description": "List and scan exposed GCS files for sensitive patterns (keys, tokens, credentials).",
        "tags": ["gcp", "gcs", "cloud", "secrets", "scan"],
    }
    ...
    def run(self):
        ...
        print_info(f"Scanning GCS patterns in {bucket}")
    ...
    def get_graph_nodes(self, data):
        if not isinstance(data, dict) or "error" in data:
            return [], []
        target = data.get("target", "gcs-sensitive-scan")
        nodes = [{
            "id": target,
            "label": target,
            "group": "hostname",
