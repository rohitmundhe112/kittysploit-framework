...
class Module(Auxiliary, Http_client):
    __info__ = {
        "name": "GCP GCS File Download",
        "author": ["KittySploit Team"],
        "description": "Download a file from a GCS bucket via anonymous access.",
        "tags": ["gcp", "gcs", "cloud", "download"],
    }

    target = OptString("", "GCS bucket name or URL", required=True)
    key = OptString("", "GCS object key (e.g. backups/db.sql)", required=True)
    output_path = OptString("", "Local output path (default: output/gcs_<bucket>_<filename>)", required=False)
    ...
    def _default_output_path(self, bucket, key):
        ...
        return os.path.join("output", f"gcs_{safe_bucket}_{key_name}")
    ...
    def run(self):
        ...
        url = f"https://storage.googleapis.com/{bucket}/{quote(key, safe='/')}"
        print_info(f"Downloading gs://{bucket}/{key}")
