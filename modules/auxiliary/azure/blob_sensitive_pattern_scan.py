...
class Module(Auxiliary, Http_client):
    __info__ = {
        "name": "Azure Blob Sensitive Pattern Scan",
        "author": ["KittySploit Team"],
        "description": "List and scan exposed blob files for sensitive patterns (keys, secrets, credentials).",
        "tags": ["azure", "cloud", "storage", "secrets", "scan"],
    }
    ...
    SENSITIVE_PATTERNS = [
        ...
        ("connection_string", re.compile(r"(?i)(AccountKey=|DefaultEndpointsProtocol=|EndpointSuffix=|SharedAccessSignature=)")),
    ]
    ...
    def _list_blobs(self, account, container, prefix, sas, timeout_seconds, max_files):
        ...
        while True:
            ...
            page += 1
            if page % 5 == 0:
                print_status(f"Listing progress: pages={page}, files_discovered={len(files)}")
    ...
    def run(self):
        ...
        print_info(f"Scanning blob patterns in {account}/{container}")
        files = self._list_blobs(account, container, prefix, sas, timeout_seconds, max_files)
        if not files:
            print_warning("No files listed or inaccessible container.")
            return {
        ...
        print_info(f"Files discovered for scan: {len(files)}")
        ...
        for name in files:
            ...
            if q:
                q = "?" + q
            url = f"https://{account}.blob.core.windows.net/{container}/{name}{q}"
            resp = self._http_get_url(url, timeout_seconds)
            scanned += 1
    ...
    def get_graph_nodes(self, data):
        if not isinstance(data, dict) or "error" in data:
            return [], []
        target = data.get("target", "azure-blob-scan")
        nodes = [{
            "id": target,
            "label": target,
            "group": "hostname",
