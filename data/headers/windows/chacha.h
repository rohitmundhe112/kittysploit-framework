...
static void chacha_keysetup(chacha_ctx *x, const unsigned char *k, unsigned long keybits, unsigned long ivbits)
{
    ...
    x->input[0] = *(DWORD *)(constants + 0);
    x->input[1] = *(DWORD *)(constants + 4);
    x->input[2] = *(DWORD *)(constants + 8);
    x->input[3] = *(DWORD *)(constants + 12);
    if (keybits == 256) {
        x->input[4] = *(DWORD *)(k + 0);
        x->input[5] = *(DWORD *)(k + 4);
        x->input[6] = *(DWORD *)(k + 8);
        x->input[7] = *(DWORD *)(k + 12);
        x->input[8] = *(DWORD *)(k + 16);
        x->input[9] = *(DWORD *)(k + 20);
        x->input[10] = *(DWORD *)(k + 24);
        x->input[11] = *(DWORD *)(k + 28);
    } else {
        x->input[4] = *(DWORD *)(k + 0);
        x->input[5] = *(DWORD *)(k + 4);
        x->input[6] = *(DWORD *)(k + 8);
...
static void chacha_ivsetup(chacha_ctx *x, const unsigned char *iv)
{
    x->input[12] = 0;
    x->input[13] = 0;
    x->input[14] = *(DWORD *)(iv + 0);
    x->input[15] = *(DWORD *)(iv + 4);
}
